        ...
        <div className="mr-[4rem] mt-[5rem]">
          ...
          <form onSubmit={submitHandler} className="container w-[40rem] class-page-login-reg">
            ...
            <div className="mb-4">
              ...
              <button type="button" onClick={capture} className="mt-2 bg-blue-500 text-white px-4 py-2 rounded">Capture Image</button>
              {image && <img src={image} alt="User" className="mt-2 rounded w-full" />}
            </div>
            <div className="mb-4">
              <button type="button" onClick={getLocation} className="bg-green-500 text-white px-4 py-2 rounded">Get Location</button>
              {location && <p className="text-white mt-2">Location: {location.latitude}, {location.longitude}</p>}
            </div>
            <button disabled={isLoading} type="submit" className="bg-[#5f2476] text-white px-4 py-2 rounded cursor-pointer my-[1rem]">{isLoading ? "Signing In..." : "Sign In"}</button>
            {isLoading && <Loader />}
          </form>
          <div className="mt-4">
            <p className="text-white">New Customer? <Link to={redirect ? `/register?redirect=${redirect}` : "/register"} className="text-[#5f2476] hover:underline">Register</Link></p>
          </div>
        </div>
